...
"""
ML Pipeline Part 1: Preprocessing and Clustering for Rental Properties

This script creates distinct property clusters from the rental dataset.
...
ENCODERS_FILENAME = "rental_encoding_maps.json"
SCALER_FILENAME = "rental_scaler.pkl"
IMPUTER_FILENAME = "rental_iterative_imputer.pkl"
KMEANS_MODEL_FILENAME = "rental_kmeans_model.pkl"
...
def preprocess_and_encode(df, target_variable, encoders_filepath, imputer_filepath):
    ...
    # Step 2e: Handle Boolean and Categorical Amenity Columns

    # For 'acepta_mascotas', we will fill NaNs with False, as requested.
    # This means "not specified" is treated as "not allowed".
    if 'acepta_mascotas' in df.columns:
        print("Processing 'acepta_mascotas': Filling NaNs with False.")
        df['acepta_mascotas'] = df['acepta_mascotas'].fillna(False)
    
    # For 'amueblado', we still want to keep NaN as a separate category.
    # So, we one-hot encode it as before.
...
if __name__ == "__main__":
    ...
    if df_rental is not None:
        ...
        cluster_label_map = {
            0: 'Residencial Familiar con Extras',
            1: 'Piso Básico y Económico',
            2: 'Apartamento Céntrico Amueblado',
            3: 'Vivienda de Lujo Exclusivo',
            4: 'Apartamento Premium (Ubicación Prime)'
        }
